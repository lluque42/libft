...
/*   Updated: 2024/07/08 01:24:12 by lluque           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	add_sign_flag(t_format *format, char **nbr)
{
	...
	if (format->data_type != INT && format->data_type != DECIMAL)
		return (1);
	if (ft_printf_has_flag(format->flags, BLANK_IF_POS_NUM | SIGN_BEF_NUM))
	{
		...
		if (ft_printf_has_flag(format->flags, SIGN_BEF_NUM))
			ft_strlcat(new_nbr, "+", new_nbr_size);
		if (ft_printf_has_flag(format->flags, BLANK_IF_POS_NUM))
			ft_strlcat(new_nbr, " ", new_nbr_size);
...
int	ft_printf_print_integer(t_format *format, va_list *arg_list)
{
	...
	if (format->data.int_type >= 0 && !add_sign_flag(format, &nbr))
		return (ft_printf_free_and_return_value(nbr, 0));
	if (!ft_printf_add_precision_for_numbers(format, &nbr))
		return (ft_printf_free_and_return_value(nbr, 0));
	if (!ft_printf_add_width_alignment(format, &nbr))
		return (ft_printf_free_and_return_value(nbr, 0));
